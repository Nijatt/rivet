...
# engine.gl_camera.screen_to_world_ray();
...
while running:
    ...
    cam_pos, ray_dir = engine.gl_camera.screen_to_world_ray(mouse_pos[0], mouse_pos[1],[w,h], view_matrix, proj_matrix)
    ...
    if mouse_buttons[0]:  # left click held
        if selected_particle is None:
            ...
            for p in elastic_rod.particles:
                ...
                dist = np.linalg.norm(closest_point - p.pos)
            ...
            if selected_particle.inv_mass > 0:
                depth = np.dot(selected_particle.transform.position - cam_pos, ray_dir)
                selected_particle.transform.position = cam_pos + depth * ray_dir + drag_offset
